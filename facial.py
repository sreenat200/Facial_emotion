...
with st.sidebar:
    ...
    mirror = st.checkbox("Mirror Video", value=True)  # Option for mirroring
...
class EmotionProcessor(VideoProcessorBase):
    def __init__(self, mirror=False):
        ...
        self.mirror = mirror  # Store mirror setting
...
try:
    webrtc_streamer(
        ...
        video_processor_factory=lambda: EmotionProcessor(mirror=mirror),  # Pass mirror setting
...
```To add an option for mirroring the camera feed in the provided Streamlit code, we'll introduce a toggle in the Streamlit sidebar to let the user choose whether to mirror the video feed (horizontally flip the image) for either camera (camera 1 or camera 0). This will modify the `EmotionProcessor` class to use a `mirror` flag, controlled by a Streamlit checkbox, instead of hardcoding mirroring based on the camera ID. The rest of the code will remain unchanged to preserve the facial emotion detection functionality.

Below is the fully updated code with the mirroring option added:
...
with st.sidebar:
    ...
    quality = st.selectbox("Select Video Quality", ["Low (480p)", "Medium (720p)", "High (1080p)"], index=2)  # Default to High
    fps = st.selectbox("Select FPS", [15, 30, 60], index=2)  
    mirror_feed = st.checkbox("Mirror Video Feed", value=True)  # Checkbox for mirroring, default True
...
class EmotionProcessor(VideoProcessorBase):
    def __init__(self, mirror=False):
        self.face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        self.mirror = mirror  # Store mirroring preference

    def recv(self, frame):
        img = frame.to_ndarray(format="bgr24")
        
        # Apply mirroring if the mirror flag is True
        if self.mirror:
        ...
        for (x, y, w, h) in faces:
            ...
            text_size = cv2.getTextSize(emotion, cv2.FONT_HERSHEY_SIMPLEX, 0.9, 2)[0]
            cv2.rectangle(img, (x, y-35), (x+text_size[0], y-5), (255, 255, 255), -1)  
            cv2.putText(img, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
...
try:
    webrtc_streamer(
        key="emotion-detection",
        video_processor_factory=lambda: EmotionProcessor(mirror=mirror_feed),  # Pass mirror setting
        media_stream_constraints={
            "video": {
                "width": {"ideal": resolution["width"]},
                "height": {"ideal": resolution["height"]},
